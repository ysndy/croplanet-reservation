...
import croplanet.admin.domain.RequestDomain;
...
public class AdminController {
    ...
    @GetMapping()
    public String chart(@ModelAttribute Log log, Model model){
...
//        model.addAttribute("requestDomains", requestDomains);
        ...
        //모든 요청 저장
        List<String> logs = logService.getRequests();

        //요청에 대한 값들 저장
        List<Long> countsByLogs = logs.stream()
                .map(r -> logService.getCountByRequest(r))
                .collect(Collectors.toList());
        ...
        //모든 날짜 저장
        List<String> dateTimes = logService.getAllDates();
